...
int n;
int dp[7][7500];
ll FindNumberOfways(int indx , int rem){
    if( rem< 0  || indx > 5 )re 0 ;
    if(rem == 0 ) re 1 ;
    ll z =0 ;
    if(dp[indx][rem] != -1 ) re dp[indx][rem];
    z+=FindNumberOfways(indx, rem - v[indx]) ;
    z+=FindNumberOfways(indx+1 , rem);
    re  dp[indx][rem] = z;
}   
...
//    cin >> tc;
   ...
   int dump=  FindNumberOfways(1 ,7499 );
    while(cin  >> n ){
        cout << FindNumberOfways(1 , n) << el;
    }
