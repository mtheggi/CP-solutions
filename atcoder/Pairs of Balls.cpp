...
void EZ() {
    int n , m, k ,a  ;
    cin >> n >> m;
    vector<queue<int>> cylinders(m+1 );
    ...
    for(int i =1 ;i <=m ; i++ ) {
        ...
        colorsIndices[cylinders[i].front()].push_back(i);
    }
    queue<int> q;
    for(int i =1 ;i <=n ; i++) {
    ...
    while(!q.empty()){
        ...
        for(int i =0 ; i< colorsIndices[front].size() ; i++ ){
            int position =colorsIndices[front][i];
            cylinders[position].pop();
            if(cylinders[position].size()) {
                int color = cylinders[position].front();
                colorsIndices[color].push_back(position);
                if(colorsIndices[color].size() == 2) q.push(color);
            }
        }

    }
    for(auto i : cylinders) if(i.size()) {NO;re;}
    YES;
}
